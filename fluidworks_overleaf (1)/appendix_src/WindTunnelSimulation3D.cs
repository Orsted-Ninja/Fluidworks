...
public class WindTunnelSimulation3D : MonoBehaviour
{
    ...
    void ApplyVisualizationState(string mode)
    {
        ...
        if (flowParticles != null)
        {
            ...
            flowParticles.ApplyVisualizationMode(normalized);
        }

        streamlineRenderer?.ApplyVisualizationMode(normalized);
        streamlineFieldRenderer?.ApplyVisualizationMode(normalized);
        flowFieldSliceRenderer?.ApplyVisualizationMode(normalized);

        // Surface Friction mode: set surfaceMode on all SurfacePressureVisualizers
        bool frictionMode = string.Equals(normalized, VisualizationSurfaceFriction, System.StringComparison.OrdinalIgnoreCase);
        bool pressureMode = string.Equals(normalized, VisualizationPressure, System.StringComparison.OrdinalIgnoreCase);
        ...
        for (int i = 0; i < surfaceVisualizers.Length; i++)
        {
            ...
            if (frictionMode)
            {
                surfaceVisualizers[i].surfaceMode = AeroFlow.Visualization.SurfacePressureVisualizer.SurfaceMode.Friction;
                surfaceVisualizers[i].enabled = true;
            }
            else if (pressureMode)
            {
                surfaceVisualizers[i].surfaceMode = AeroFlow.Visualization.SurfacePressureVisualizer.SurfaceMode.Pressure;
                surfaceVisualizers[i].enabled = true;
            }
            else
            {
