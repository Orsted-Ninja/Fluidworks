...
public class NavierStokesGridSolver : MonoBehaviour
{
    ...
    const int MaxObstacleSpheres = 64;
    ...
    public bool TryGetPressureRange(out float minPressure, out float maxPressure)
    {
        ...
        if (!diagnostics.valid || diagnosticsPressureCache == null)
        {
            return false;
        }

        int count = diagnosticsPressureCache.Length;
        if (count == 0)
        {
            return false;
        }

        minPressure = float.PositiveInfinity;
        maxPressure = float.NegativeInfinity;
        for (int i = 0; i < count; i++)
        {
            float p = diagnosticsPressureCache[i];
            if (!float.IsFinite(p))
            {
                continue;
            }
        ...
        if (!float.IsFinite(minPressure) || !float.IsFinite(maxPressure))
        {
            minPressure = 0f;
            maxPressure = 0f;
            return false;
        ...
        float deltaTime)
    {
        ...
        bool shouldRefreshDiagnostics = !diagnostics.valid
            || Time.time >= nextDiagnosticsTime
    ...
    void BindBuffers()
    {
        ...
        if (obstacleVelocityBuffer != null)
        {
            ComputeHelper.SetBuffer(navierShader, obstacleVelocityBuffer, "ObstacleVelocity", applyForcesKernel, advectVelocityKernel, vorticityKernel, projectKernel);
        }
    }
    ...
    void ReleaseBuffers()
    {
        ComputeHelper.Release(gridVelocity, gridVelocityTmp, gridPressure, gridPressureTmp, gridDivergence, obstacleSphereBuffer, obstacleMaskBuffer, obstacleVelocityBuffer);
        gridVelocity = null;
    ...
    void UpdateObstacleProxyData(Transform obstacleRoot, Vector3 obstacleCenter, Vector3 obstacleSize)
    {
        ...
        if (obstacleVelocityBuffer != null)
        {
            ...
            var registry = obstacleRoot != null ? obstacleRoot.GetComponent<AeroFlow.Core.PartRegistry>() : null;
            if (registry != null)
            {
                ObstacleVoxelizer.BuildVelocityField(registry, domainBounds, gridSizeX, gridSizeY, gridSizeZ, obstacleMaskData, velocities);
