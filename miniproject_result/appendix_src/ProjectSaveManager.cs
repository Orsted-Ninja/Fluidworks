    ...
    public class ProjectSaveManager : MonoBehaviour
    {
        ...
        private IEnumerator SaveRoutine(string targetArchiveFile, ProjectData projectData, string sourceModelFilePath)
        {
            ...
            try
            {
                ...
                // 3.5 Include Generated Report if it exists
                string reportPdfSource = Path.Combine(Application.persistentDataPath, "GeneratedReport.pdf");
                if (File.Exists(reportPdfSource))
                {
                    File.Copy(reportPdfSource, Path.Combine(tempDir, "GeneratedReport.pdf"), true);
                    Debug.Log("[SaveManager] Included GeneratedReport.pdf in project archive.");
                }
